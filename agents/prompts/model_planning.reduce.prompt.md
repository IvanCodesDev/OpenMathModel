...
variant: reduce
version: 2
input_schema: {"type": "object", "required": ["proposals", "problem_analysis", "data_profile", "implementation_languages"], "properties": {"proposals": {"type": "string"}, "problem_analysis": {"type": "string"}, "data_profile": {"type": "string"}, "implementation_languages": {"type": "string"}}}
output_schema: {"type": "object", "required": ["plans", "recommended_plan_id", "rationale"], "properties": {"plans": {"type": "array", "items": {"type": "object", "required": ["id", "name", "approach", "steps", "risks", "role", "source_views"], "properties": {"id": {"type": "string", "enum": ["A", "B", "C"]}, "name": {"type": "string"}, "approach": {"type": "string"}, "steps": {"type": "array", "items": {"type": "string"}}, "risks": {"type": "array", "items": {"type": "string"}}, "role": {"type": "string", "enum": ["primary", "baseline", "fallback"]}, "source_views": {"type": "array", "items": {"type": "string"}}, "fallback_condition": {"type": "string"}, "language": {"type": "string"}}}}, "recommended_plan_id": {"type": "string", "enum": ["A", "B", "C"]}, "rationale": {"type": "string"}, "dropped": {"type": "array", "items": {"type": "string"}}, "progress_note": {"type": "string"}}}
---
...
5. 每张卡给出 `language`（实现语言，小写标识如 `python`）：**只能从上面「当前可用的实现语言」里选**，按该方法最顺手的工具链取舍；列表里只有一种时就填它。用户在确认方案时会一并确认语言，之后的实验代码按它生成。

## 输出要求

只输出一个 JSON 对象，不要任何解释文字或 Markdown 代码围栏，字段如下：

- `plans`：按 A、B、C 顺序的方案卡数组（1-3 张），每张含 `id`、`name`、`approach`、`steps`、`risks`、`role`、`source_views`、`language`，`role` 为 `fallback` 时另含 `fallback_condition`。
