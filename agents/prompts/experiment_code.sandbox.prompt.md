...
4. 关键结果写入当前目录文件：至少一个 `results.csv`（结果表）；结果图 1-3 张保存到当前目录（matplotlib 可用时存 `.png`，否则用手写 SVG 字符串存 `.svg`）——这些图会作为论文的真实图件被引用，所以：文件名用能说明内容的英文短语（如 `fit_vs_baseline.png`、`convergence_curve.png`），每张图带标题与坐标轴标签（含单位），只画真实计算结果、不画示意图；同一文件名只写一次。
