...
> [!IMPORTANT]
> 项目仍在持续开发中。当前 14 个 Web 页面及其视觉、布局和交互是产品界面基线；后续开发只在现有页面中接入真实数据，不另建替代页面。登录与账号安全已有 API 对接；项目、任务、步骤、审批（含 G1 方案门、G2 数据闸门与完成后修订回合）、SSE 事件、Artifact、附件解析（扫描件走远程 OCR）、五类阶段正文投影和论文导出 API 均已实现并接入现有页面；论文编辑保存与独立 Worker 执行面尚未接入。
...
| 模块 | 状态 | 已有能力 |
|---|---|---|
| Web | 可运行 | React + TypeScript + Vite，使用现有路由表、页面模板和 DOM 适配器承载 14 个产品页面；首页发送已真实创建 Project/TaskRun 并传播 `run_id`（`/confirm` 为直接访问的草稿复核入口），工作台状态、审批（含修订门）、SSE、Artifact 与五类阶段正文投影已原位接通；论文编辑保存（乐观锁）仍待接入 |
...
#   页面上表现为思考行标「本次调用中断」、阶段反复自动重跑却永远跑不完；API 启动日志
#   会以 ERROR（omm.reload_guard）指出这一配置并给出修正命令，页面上不显示开发细节。
...
这条 API 流程用于独立诊断登录、Project、TaskRun 与 workspace 契约。产品页面中的“首页输入 → 发送（登录续接）→ 创建运行 → 任务执行页”也已使用同一组接口，`/confirm` 保留为直接访问时的草稿复核入口；附件已支持二进制上传与服务端文本抽取（扫描件经远程 OCR，需配置 `OMM_OCR_API_*`，见 `backend/api/.env.example`）。
