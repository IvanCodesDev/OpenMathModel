...
# 因此分组顺序即优先级：本地零费用 → 当代具体型号 → 上一代型号 → 家族兜底。
# 新模型上线时只需往「当代」组补一行，未补也能落到家族兜底，不会算不出钱。

PRICING: tuple[tuple[str, float, float], ...] = (
    # 本地推理：无 API 费用。放在最前，免得 qwen3:8b 这类本地标签先被
    # 下面的 qwen 家族兜底价命中。
    ("ollama", 0.0, 0.0),
    ("llama", 0.0, 0.0),
    ("qwen3:", 0.0, 0.0),
    ...
    ("gpt-5.6", 35.0, 210.0),
    # Anthropic：Mythos 5 与 Fable 5 同规格
    ("claude-fable-5", 70.0, 350.0),
    ("claude-mythos-5", 70.0, 350.0),
    ("claude-opus-5", 35.0, 175.0),
    # Google：3.6 Flash 用标准价，年底前的折扣期实际更低
    ("gemini-3.6-flash", 10.5, 52.5),
    # 阿里通义：3.8-Flash 官方直接公布人民币价
    ("qwen3.8-flash", 1.0, 3.0),
    # 智谱：GLM-5.3 与 GLM-5.2 同价
    ("glm-5.3", 9.8, 30.8),
    ("glm-5.2", 9.8, 30.8),
    # xAI：4.5/4.6 同价，提示超过 20 万 token 时翻倍（此处不建模）
    ("grok-4.6", 14.0, 42.0),
    ("grok-4.5", 14.0, 42.0),
    # ── 上一代模型：厂商已下线但中转站普遍还在供，保留以免估成兜底价 ──
    ...
    ("qwen-turbo", 0.3, 0.6),
    ("glm-4-flash", 0.0, 0.0),
    # ── 家族兜底：新型号未收录时按该家族当前主力档估算 ──────────────
    # 官方未公布单价的档位（Sonnet 5 / Haiku 4.5 / Qwen3.8-Max / Kimi K3 等）
    # 落在这里，沿用同家族上一代的档位价。轻量档若与旗舰同名前缀（如
    # GLM-5.3-Flash 命中 glm-5.3）会被高估，这个方向对预算闸门是安全的。
    ("haiku", 5.6, 28.0),
    ("claude", 21.0, 105.0),
    ("deepseek", 2.0, 8.0),
    ("gemini", 10.5, 52.5),
