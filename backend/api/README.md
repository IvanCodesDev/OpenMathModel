...
.venv\Scripts\python -m pip install -e packages/contracts -e "backend/api[dev]"

# 2) 启动（默认 SQLite：backend/api/data/dev.db，启动时 create_all 自动建表）
cd backend/api
