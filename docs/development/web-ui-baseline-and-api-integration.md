...
2026-09-19 优秀论文页经业主授权按指定截图更新：搜索区下方使用比赛、奖项、题组三组弧形轮盘，每组必须是上下边缘连续弯曲的完整弧带，不得以错位或旋转的矩形卡片替代。根据业主后续复核，选中态改为贯穿弧带的纯白分区，不使用白色圆角卡片、描边或阴影；常规轮盘中央白色分区固定，文字与分隔线沿曲面滑动。连续点击从当前可见位置接续运动，不重建跳回起点。只有 1–3 个选项时均分弧带，白色选区平滑移动，不重复填充选项；窄屏常规轮盘展示三格以给名称留足空间。长英文名称按实际词宽缩小字号，其他长文本自然换行，不截断；其他真实选项保留在轮盘环上。左右箭头与侧边选项滑动切换，支持键盘方向键与减少动态效果设置。已选条件仅展示实际生效的比赛、奖项、题组、年份与搜索词，可逐项清除；数量来自完整论文目录的真实筛选结果。保留论文内容、年份下拉、比赛关联选项范围、分页、收藏和正文路由；样式限于论文目录页面。默认无筛选时显示“未设置筛选条件”。
...
2026-09-18 设置中心补充：业主授权设置弹框同步当前冷灰配色，仅调整颜色、边框颜色和阴影，保持八个分类页、内容、布局与设置行为不变。

2026-09-18 同日复验：按业主进一步要求，正文面板右上角改为直角；对话标题底线、标题操作按钮与侧栏筛选按钮底色移除。赛题库、优秀论文和方法库仅调整留白、列表密度、筛选换行与方法标题操作区排版，内容与功能保留（ADR-0022 同日补充）。以下初始圆角说明以此补充为准。

2026-09-18 经业主授权（ADR-0022），全站浅色界面采用所提供截图的冷灰配色、白色正文面板和 32px 外层圆角。仅调整颜色、容器外形及正文面板顶部留边；现有内容、素材、字体、列宽、路由、DOM 槽位和交互顺序保持不变。共享外观位于 `apps/web/src/reference-theme.css`，在现有页面样式之后、无障碍覆盖之前加载。深色主题继续使用原有配色，无障碍偏好优先生效。
