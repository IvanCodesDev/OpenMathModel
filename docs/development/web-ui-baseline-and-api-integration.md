...
- 状态：当前规范（Normative）
- 日期：2026-08-09（2026-09-05 增补 §2.2 默认空态）
- 关联：ADR-0006、ADR-0009、ADR-0014、`packages/contracts`、`backend/api`
...
### 2.2 默认空态与演示态（2026-09-05，ADR-0014）

上述视觉基线描述的是**骨架**，不是页面自带的那套「2026 国赛 A 题 / 城市共享单车调度优化」演示内容。自 ADR-0014 起：

- **默认态**（无 `run_id`、无 `demo=1`）：页面只渲染骨架与一句空态说明，不渲染任何虚构数据。上文截图对应的是演示态，不再是默认观感。
- **演示态**：URL 带 `?demo=1` 时渲染与 ADR-0014 之前完全一致的演示内容，用于截图、演示与界面回归对照。判定统一走 `apps/web/src/integration/demo-mode.ts` 的 `demoMode()`。
- **真实运行**：由控制器把契约数据填进骨架，行为不变。

因此**界面基线截图需要按三套状态维护**：默认空态、`?demo=1`、真实 `run_id`。任何新增页面内容都必须能回答「这段文字的数据源是什么」——没有数据源的内容不写进模板。

骨架里供控制器定位的稳定选择器是硬约束，空态下同样必须存在（详见 ADR-0014 决策 3）。空态下默认 `hidden` 的槽位，由对应渲染器在填入真实内容时显式放出。

已随 ADR-0014 删除、不再是基线一部分：确认页「任务目标 / 输出要求 / 执行方式」三组静态清单；数据页「原始数据 / 清洗数据」两个纯演示次级页面（仅 `?demo=1` 保留）。方案页「模型假设 / 符号表」在 ADR-0014 成文当日随 H3 切片 2 获得真实数据源（`plan-proposal` 契约的 `assumptions` / `symbols`，方案阶段归约后的规范化产出），与「字段说明 / 实现计划」同一纪律：默认入口隐藏，由 `stage-content.ts` 的 `renderAssumptionsPanel` / `renderSymbolsPanel` 在内容就绪后放出；契约字段为 null（切片 2 之前的运行、规范化失败）时入口保持隐藏。§2.1 第十七轮所述「补齐 10 个次级标签页面」相应收敛为 8 个有真实数据源的次级页面。

输入框上的模型选择器同样按此规则收敛：默认只列 `Auto` 与本机设置里确实填过的自定义 API，四条「官方服务」演示选项退到 `?demo=1`（它们本就不参与请求路由，选中不生效，详见 ADR-0014 决策 7）。

**运行态路由需要登录**：`/task/running` 与五条工作台路由靠 `run_id` 拉后端快照，未登录必然 401。这六条路由由 `apps/web/src/integration/auth-guard.ts` 在 `activateScreen` 最前面把关，未登录时不进入页面，直接跳 `/login?next=<原地址>`（ADR-0014 决策 8）。因此这六条路由的**默认空态截图必须在登录状态下拍**，未登录访问的预期结果是登录页而不是空态页。

设置中心 → 模型厂商 → 智能路由的四个下拉不再内置厂商型号：候选 = `自动选择` + 用户已保存的接口（取值 `endpoint-<id>`），并随「自定义 API」里的增删改、设主接口、调权重即时刷新（ADR-0014 决策 11）。四个下拉与「启用模型智能路由」开关随「保存更改」以 `task_routes` / `smart_routing` 进入 `PUT /api/account/llm-config`，打开面板时以服务端为准回填；服务端据此让任务的六个阶段按任务类型取调用链、携图对话走「视觉理解」定向（ADR-0015）。这是在既有 DOM 槽位内接契约，不改页面结构。设置中心「自定义 API」表单的预填示例（`api.example.com` 等）保留，它是示例占位而非虚构结论。

设置中心 → 模型厂商的卡片副标题（各厂商在售型号）、「配置」一键填入的默认模型、「默认模型 ID」补全与「用量监控」的单价，自 2026-09-06 起来自服务端同步的模型目录 `GET /api/llm/catalog`（ADR-0017，数据源为公共目录 models.dev，按 TTL 刷新、落文件缓存、失败回落上一份或内置快照），页面里不再写死任何模型名；区块标题下的新鲜度行与「同步型号」按钮是在既有区块标题槽位内的数据填充与动作补充，不改卡片结构。前端 `integration/llm-providers.ts` 只保留品牌骨架（名字 / 图标 / 协议 / 地址），`integration/model-modality.ts` 的命名规则表与后端 `llm.py` 的能力推断记号仍是「目录没有时的兜底」数据表，更新它们不构成界面改动。
...
- [ ] 状态按钮符合 TaskRun 生命周期；
- [ ] 类型检查、ESLint、生产构建通过；
- [ ] 固定视口截图与基线逐页对比（默认空态 / `?demo=1` / 真实 `run_id` 三套，见 §2.2）；
- [ ] 默认态页面不出现任何虚构数据：新增的每段内容都能指认数据源；
